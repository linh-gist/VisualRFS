...
namespace lap {
	...
	public:
		...
		std::tuple <Eigen::MatrixXd, Eigen::VectorXd> draw_solutions(CostMatrix C, int num_solutions) {
			...
			queue = std::priority_queue<MurtyStatePtr, std::vector<MurtyStatePtr>, MurtyStatePtrCompare>();
			queue.emplace(std::make_shared<MurtyState>(C));

			m_solutions.resize(num_solutions, C.rows());
			m_costs.resize(num_solutions);
			int num = 0;
			for (num = 0; num < num_solutions; num++) {
				Assignment sol;
				double cost;
				bool ok = draw(sol, cost);
				if (ok) {
					m_solutions.row(num) = sol.cast<double>();
					m_costs(num) = cost;
				}
				else {
					break;
			...
			return { m_solutions(Eigen::seq(0, num - 1), Eigen::all), m_costs(Eigen::seq(0, num - 1)) };
		}

	private:
		std::priority_queue<MurtyStatePtr, std::vector<MurtyStatePtr>, MurtyStatePtrCompare> queue;
		Eigen::MatrixXd m_solutions;
