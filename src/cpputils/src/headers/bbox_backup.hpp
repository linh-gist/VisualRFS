...
tuple<VectorXi, vector<MatrixXb>> extract_tt_segmentation(MatrixXd tt_xyah, VectorXi tt_label, vector<MatrixXb> tt_mask) {
    unsigned int N = tt_xyah.rows();
    if (N == 0) {
        return {tt_label, tt_mask};
    }
    tt_xyah.col(1) -= tt_xyah.col(3) / 2;  // top
    tt_xyah.col(3) += tt_xyah.col(1);      // bottom
    // sort from lower to higher bottom
    std::vector<double> bottom(N);
    VectorXd::Map(&bottom[0], N) = tt_xyah.col(3);
    std::vector<int> idx(tt_xyah.rows());
    iota(idx.begin(), idx.end(), 0);
    stable_sort(idx.begin(), idx.end(), [&bottom](int i1, int i2) {return bottom[i1] < bottom[i2];});

    MatrixXd tt_mIoU(N, N);
    tt_mIoU.setZero();
    MatrixXi tt_mIoU_check(N, N);
    tt_mIoU_check.setZero();
    float mIoU;
    for(int i=0; i<N; i++) {
        for(int j=0; j<N; j++){
            if ( i == j || tt_mIoU_check(i, j) > 0) { continue; }

            MatrixXi intersection = (tt_mask[i].array() && tt_mask[j].array()).cast<int>();
            MatrixXi union_mask = (tt_mask[i].array() || tt_mask[j].array()).cast<int>();
            mIoU = (intersection.sum() * 1.0) / union_mask.sum();
            tt_mIoU(i, j) = mIoU;
            tt_mIoU(j, i) = mIoU;
            tt_mIoU_check(i, j) = 1;
            tt_mIoU_check(j, i) = 1;
            ...
            if (mIoU > 0.9) {
                if (tt_label(i) > tt_label(j)){
                    tt_mask[j] = tt_mask[i].array() || tt_mask[j].array();
                    tt_mask[i].setConstant(false);
//                    tt_label(i) = tt_label(j);
                } else {
                    tt_mask[i] = tt_mask[i].array() || tt_mask[j].array();
                    tt_mask[j].setConstant(false);
//                    tt_label(j) = tt_label(i);
                }
            }
        }
    }

    int img_height = tt_mask[0].rows();
    int img_width = tt_mask[0].cols();
    MatrixXi id2mask(img_height, img_width);
    id2mask.setZero();
    id2mask = id2mask.array() - 1;
    for(int tt_idx : idx) {
        for(int i_h=0; i_h < img_height; i_h++){
            for(int i_w=0; i_w < img_width; i_w++) {
                if (tt_mask[tt_idx](i_h, i_w)) {
                    id2mask(i_h, i_w) = tt_label(tt_idx);
                }
            }
        }
    }
    // return new masks that are not mutual overlapping pixels
    std::vector<MatrixXb> new_tt_mask;
    std::vector<int> new_tt_idx;
    for(int tt_idx = 0; tt_idx < N; tt_idx++) {
        MatrixXb new_mask = (id2mask.array() == tt_label(tt_idx));
        if(new_mask.any()) {
            new_tt_mask.push_back(new_mask);
            new_tt_idx.push_back(tt_idx);
        }
    }
    return {VectorXi::Map(new_tt_idx.data(), new_tt_idx.size()), new_tt_mask};
}
...
//	box_w = box(2) * box(3);
	...
	box_l = box(0) - box(2) / 2;   // 1
	box_b = box(1) + box(3) / 2;  // 2
	box_r = box(0) + box(2) / 2;   // 3
